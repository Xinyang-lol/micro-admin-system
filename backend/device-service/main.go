...
import (
	...
	"micro-admin-system/backend/device-service/config"
	"micro-admin-system/backend/device-service/repository"
	devicesvc "micro-admin-system/backend/device-service/service"
...
func main() {
	...
	pb.RegisterDeviceServiceServer(server, devicesvc.New(repository.New(database)))
	...
	if err := consul.Register(ctx, cfg.ServiceName, cfg.ServiceID, cfg.Host, cfg.GRPCPort, []string{"grpc", "device"}); err != nil {
