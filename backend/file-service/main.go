...
import (
	...
	"micro-admin-system/backend/file-service/config"
	"micro-admin-system/backend/file-service/repository"
	filesvc "micro-admin-system/backend/file-service/service"
...
func main() {
	...
	pb.RegisterFileServiceServer(server, filesvc.New(repository.New(database)))
	...
	if err := consul.Register(ctx, cfg.ServiceName, cfg.ServiceID, cfg.Host, cfg.GRPCPort, []string{"grpc", "file"}); err != nil {
